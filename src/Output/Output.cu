...
void Output::writeParticles(const std::vector<Spherical> &particles, const int timestep) {
    ...
    // XML Header with proper indentation
    ...
    // Vertices section - defines each point as a vertex
    vtpFile << "      <Verts>\n";
    ...
    // Point Data section with attributes
    vtpFile << "      <PointData Scalars=\"Radius\" Vectors=\"Velocity\">\n";

    // Radius attribute - explicitly marked as the default scalar
    vtpFile << "        <DataArray type=\"Float32\" Name=\"Radius\" format=\"ascii\">\n";
    for (const auto& particle : particles) {
        vtpFile << "          " << particle.getRadius() << "\n";
    }
    vtpFile << "        </DataArray>\n";

    // Velocity attribute - explicitly marked as a vector
    vtpFile << "        <DataArray type=\"Float32\" Name=\"Velocity\" NumberOfComponents=\"3\" format=\"ascii\">\n";
    for (const auto& particle : particles) {
        vtpFile << "          "
                << particle.velocity.x << " "
                << particle.velocity.y << " "
    ...
    // ID attribute
    vtpFile << "        <DataArray type=\"Int32\" Name=\"ID\" format=\"ascii\">\n";
    for (const auto& particle : particles) {
        vtpFile << "          " << particle.getId() << "\n";
    }
    vtpFile << "        </DataArray>\n";

    // Add Mass as an attribute if available
    vtpFile << "        <DataArray type=\"Float32\" Name=\"Mass\" format=\"ascii\">\n";
    for (const auto& particle : particles) {
        vtpFile << "          " <<0 << "\n";
    }
    vtpFile << "        </DataArray>\n";
    ...
    std::cout << "Successfully wrote " << particles.size() << " spherical particles to " << filename << std::endl;
}
